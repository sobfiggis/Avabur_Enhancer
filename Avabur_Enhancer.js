        ...
        if(craft.a.m && ENABLE_DROP_TRACKER){
            ...
            if(craft.a.sr) {
                ...
                switch(/.*?>(.*?)</im.exec(craft.a.sr)[1]) {
                    case 'strength': id = 'strC'; break;
                    case 'health': id = 'heaC'; break;
                    case 'coordination': id = 'coordC'; break;
                    case 'agility': id = 'agiC'; break;
                }
                incrementCell(id);
            }
            ...
            if(craft.a.dr) {
                ...
                switch(/(Tooltip).*?>|>.*?(platinum coin|gold coin|crafting|gem frag|crystal).*?</.exec(craft.a.dr).splice(1,2).join("")) {
                    case 'Tooltip': id = "gearC"; break;
                    case 'gold coin': id = "goldC"; break;
                    case 'crafting': id = "craftC"; break;
                    case 'gem frag': id = "fragC"; break;
                    case 'crystal': id = "crystalC";
                }
                incrementCell(id);
            }
            calcPercentCells();
